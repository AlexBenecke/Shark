...
eff <- read_csv_fn( "data/raw-data/CPUE.csv", proj) %>%
  mutate(stationkey = factor(stationkey)
         ,
         year = factor(year, ordered = TRUE)
         ) %>%
  arrange(year, stationkey)
...
### Distance to shore (Skip 11-19-2018)

```{ Load packages needed to create distance to shore variable, eval=FALSE}
...
```{ Distance to Shore Variable, eval=FALSE}
...
str(eff)
  ...
  red.df <- eff %>% ### create reduced data frame
    ...
    select( stationkey, year, month, CPUE, startlat, startlon, 
            startdepth, tempbotm, turbbotm, oxybotm, salbotm, #Dis.to.Shore,
            bottype4, TAXON, fluorobotm, Adj.CPUE, start_gmt, bottype8 )
...
```{r order variables like I want, eval=FALSE}

red.df %<>% select( stationkey, year, month, common, sex, maturity, 
                  pres, CPUE, startlat, startlon, 
                  startdepth, tempbotm, turbbotm, oxybotm, salbotm, #Dis.to.Shore,
                  bottype4, fluorobotm, bottype8, Adj.CPUE, start_gmt, species, TAXON )
...
#11-19-18#write_csv_fn( red.df, file = "data/raw-data/CPUE2.csv", row.names = FALSE, proj)
...
eff2 <- read_csv_fn( "data/raw-data/CPUE2.csv", proj, na.strings = c(NA, "")) %>%
  mutate(stationkey = factor(stationkey),
         year = factor(year), 
         month = ordered(month,
                         levels = c("Jul", "Aug", "Sep", "Oct", "Nov") )
...
#### Distance to shore (Skip/removed)
...
eff2 %>% select(tempbotm, startdepth, fluorobotm, turbbotm,
                oxybotm, salbotm #, Dis.to.Shore 
                ) %>% summary()
...
```{ depth Nas, eval=FALSE, echo=FALSE,results='hide',fig.keep='last'}
...
hist(as.numeric(eff2$year[c(depth.NA)]),
     xaxt = "n",
     labels = TRUE,
     ylab = "number of NAs", 
     xlab = "Year",
...
hist( as.numeric( eff2$year[c(temp.NA)] ),
     xaxt = "n",
     labels = TRUE,
     ylab = "number of NAs", 
     xlab = "Year",
...
hist( as.numeric(eff2$year[c(fluo.NA)]),
     xaxt = "n",
     labels = TRUE,
     ylab = "number of NAs", 
     xlab = "Year",
...
rm.Turb <- which(eff2$turbbotm > 105 | eff2$turbbotm < 0.01) # 1 individual

(n.rm.turb <- length(rm.Turb)/49) ### 11 individual
...
hist( as.numeric(eff2$year[c(turb.NA)]),
     xaxt = "n",
     labels = TRUE,
     ylab = "number of NAs", 
     xlab = "Year",
...
hist( as.numeric(eff2$year[c(oxy.NA)]),
     xaxt = "n",
     labels = TRUE,
     ylab = "number of NAs", 
     xlab = "Year",
...
rm.Sal <- which(eff2$salbotm > 40 | eff2$salbotm < 10)

par(mfrow = c(1,1))
plot(CPUE ~ salbotm,
     data = eff2[-c(rm.Sal),],
     xlim = c(9, 50), ylim = c(0,70),
     pch = 19,
     col = rgb(0,0,0, 0.15),
     main = "",
     bty = "n")
...
hist( as.numeric(eff2$year[c(sal.NA)]),
     xaxt = "n",
     labels = TRUE,
     ylab = "number of NAs", 
     xlab = "Year",
...
(n.lines.rm <-   length(rm.Turb) + length(rm.Sal) #+ length(rm.temp)
 )

(n.rm <- ( length(rm.Turb) + length(rm.Sal) #+ length(rm.temp)
           )/49 ) ### individuals
...
#11-19-18#write_csv_fn( eff2, file = "data/clean-data/CPUE3.csv", row.names = FALSE, proj)
```
...
```{r Load data function, eval=FALSE}
...
Remove all NA's and fluorobotm
```{r}
source("5_functions/dropbox_fn.R") # For interacting with dropbox
proj = "shark" 
library(FSA)
...
#9-18-18#write_csv_fn( eff3, file = "data/clean-data/CPUE3.csv", row.names = FALSE, proj)
```
